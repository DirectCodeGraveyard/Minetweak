...
import net.minecraft.crafting.ICrafting;
...
import net.minecraft.player.PlayerPositionComparator;
import net.minecraft.player.Team;
import net.minecraft.player.score.Score;
import net.minecraft.player.score.ScoreObjective;
import net.minecraft.player.score.ScorePlayerTeam;
import net.minecraft.player.score.Scoreboard;
...
public abstract class ServerConfigurationManager {
    ...
    public void initializeConnectionToPlayer(INetworkManager par1INetworkManager, EntityPlayerMP par2EntityPlayerMP) {
        ...
        this.sendChatMessageToAll(ChatMessageComponent.createWithType("multiplayer.player.joined", par2EntityPlayerMP.getTranslatedEntityName()).func_111059_a(EnumChatFormatting.YELLOW));
    ...
    public void sendPacketToAllPlayersInDimension(Packet par1Packet, int par2) {
        for (EntityPlayerMP aPlayerEntityList : this.playerEntityList) {
            if (aPlayerEntityList.dimension == par2) {
                aPlayerEntityList.playerNetServerHandler.sendPacket(par1Packet);
            }
        }
    }
    ...
    /**
     * returns a string containing a comma-separated list of player names
     */
    public String getPlayerListAsString() {
        ...
        for (int var2 = 0; var2 < this.playerEntityList.size(); ++var2) {
            ...
            var1 = var1 + (this.playerEntityList.get(var2)).getCommandSenderName();
        }
     ...
     */
    public List<EntityPlayerMP> findPlayers(ChunkCoordinates par1ChunkCoordinates, int par2, int par3, int par4, int par5, int par6, int par7, Map<String, Integer> par8Map, String par9Str, String par10Str, World par11World) {
        if (this.playerEntityList.isEmpty()) {
            return null;
        } else {
            List<EntityPlayerMP> var12 = new ArrayList<EntityPlayerMP>();
            ...
            for (EntityPlayerMP aPlayerEntityList : this.playerEntityList) {
                ...
                if ((par11World == null || aPlayerEntityList.worldObj == par11World) && (par9Str == null || var14 != par9Str.equalsIgnoreCase(aPlayerEntityList.getEntityName()))) {
                    if (par10Str != null) {
                        Team var20 = aPlayerEntityList.getTeam();
                    ...
                    if (par1ChunkCoordinates != null && (par2 > 0 || par3 > 0)) {
                        float var22 = par1ChunkCoordinates.getDistanceSquaredToChunkCoordinates(aPlayerEntityList.getCommandSenderPosition());
                    ...
                    if (this.func_96457_a(aPlayerEntityList, par8Map) && (par5 == EnumGameType.NOT_SET.getID() || par5 == aPlayerEntityList.theItemInWorldManager.getGameType().getID()) && (par6 <= 0 || aPlayerEntityList.experienceLevel >= par6) && aPlayerEntityList.experienceLevel <= par7) {
                        var12.add(aPlayerEntityList);
                    }
                }
            }
            ...
            if (par4 > 0) {
                var12 = var12.subList(0, Math.min(par4, var12.size()));
            }
    ...
    private boolean func_96457_a(EntityPlayer par1EntityPlayer, Map<String, Integer> par2Map) {
        if (par2Map != null && par2Map.size() != 0) {
            Iterator<Entry<String, Integer>> var3 = par2Map.entrySet().iterator();
            Entry<String, Integer> var4;
            boolean var6;
            int var10;
            ...
            while (var10 <= var4.getValue() || var6);
    ...
    public void sendToAllNearExcept(EntityPlayer par1EntityPlayer, double par2, double par4, double par6, double par8, int par10, Packet par11Packet) {
        for (EntityPlayerMP aPlayerEntityList : this.playerEntityList) {
            ...
            if (aPlayerEntityList != par1EntityPlayer && aPlayerEntityList.dimension == par10) {
                double var14 = par2 - aPlayerEntityList.posX;
                double var16 = par4 - aPlayerEntityList.posY;
                double var18 = par6 - aPlayerEntityList.posZ;

                if (var14 * var14 + var16 * var16 + var18 * var18 < par8 * par8) {
                    aPlayerEntityList.playerNetServerHandler.sendPacket(par11Packet);
                }
            }
        }
    }
    ...
    public void saveAllPlayerData() {
        for (EntityPlayerMP aPlayerEntityList : this.playerEntityList) {
            this.writePlayerData(aPlayerEntityList);
        }
    }
    ...
    public List<ICrafting> getPlayerList(String par1Str) {
        ArrayList<ICrafting> var2 = new ArrayList<ICrafting>();

        for (EntityPlayerMP aPlayerEntityList : this.playerEntityList) {
            if (aPlayerEntityList.getPlayerIP().equals(par1Str)) {
                var2.add(aPlayerEntityList);
            }
        }
    ...
    public void removeAllPlayers() {
        while (!this.playerEntityList.isEmpty()) {
            (this.playerEntityList.get(0)).playerNetServerHandler.kickPlayer(TweakConfig.get("server"));
        }
    }

    public void sendChatMessageToAll(ChatMessageComponent par1ChatMessageComponent, boolean par2) {
