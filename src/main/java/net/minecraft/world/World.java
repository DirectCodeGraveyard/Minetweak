...
import net.minecraft.utils.callable.CallableLvl1;
import net.minecraft.utils.callable.CallableLvl2;
import net.minecraft.utils.callable.CallableLvl3;
...
public abstract class World implements IBlockAccess {
    ...
    private ArrayList<TileEntity> addedTileEntityList = new ArrayList<TileEntity>();
    /**
    ...
    /**
     * number of ticks until the next random ambient play
     */
    ...
    /**
     * ray traces all blocks, including non-collide'able ones
     */
     ...
     * aabb
     */
    public List<? extends AxisAlignedBB> getCollidingBoundingBoxes(Entity par1Entity, AxisAlignedBB par2AxisAlignedBB) {
    ...
    public boolean isBlockTopFacingSurfaceSolid(Block par1Block, int par2) {
        return par1Block != null && (par1Block.blockMaterial.isOpaque() && par1Block.renderAsNormalBlock() || (par1Block instanceof BlockStairs ? (par2 & 4) == 4 : (par1Block instanceof BlockHalfSlab ? (par2 & 8) == 8 : (par1Block instanceof BlockHopper || (par1Block instanceof BlockSnow && (par2 & 7) == 7)))));
    }
        ...
        if (var6 > 0.15F) {
            ...
            if (par2 >= 0 && par2 < 256 && this.getSavedLightValue(EnumSkyBlock.Block, par1, par2, par3) < 10) {
                ...
                if ((var7 == Block.waterStill.blockID || var7 == Block.waterMoving.blockID) && this.getBlockMetadata(par1, par2, par3) == 0) {
                    ...
                    if (this.getBlockMaterial(par1 - 1, par2, par3) != Material.water) {
                        var8 = false;
                    }
    ...
    /**
     * marks the chunk that contains this Tile Entity as modified and then calls worldAccesses.doNothingWithTileEntity
     */
     ...
     */
    public int countEntities(Class<Entity> par1Class) {
        int var2 = 0;

        for (Entity aLoadedEntityList : this.loadedEntityList) {

            if ((!(aLoadedEntityList instanceof EntityLiving) || !((EntityLiving) aLoadedEntityList).func_104002_bU()) && par1Class.isAssignableFrom(aLoadedEntityList.getClass())) {
                ++var2;
            }
        }
     ...
     */
    public WorldSavedData loadItemData(Class<? extends MapData> par1Class, String par2Str) {
    ...
    public void destroyBlockInWorldPartially(int par1, int par2, int par3, int par4, int par5) {
        for (IWorldAccess var7 : this.worldAccesses) {
